...
async def start(client, message):
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        ...
        if MOVIE_WEB_URL:
            buttons.insert(0, [InlineKeyboardButton('🎬 Movie Portal', url=MOVIE_WEB_URL)])
        await message.reply_photo(photo=random.choice(PICS), caption=script.SUR_TXT.format(message.from_user.mention, temp.U_NAME, temp.B_NAME), reply_markup=InlineKeyboardMarkup(buttons), parse_mode=enums.ParseMode.HTML)
        return
    ...
    if not files_:
        ...
        try:
            msg = await client.send_cached_media(chat_id=get_file_send_chat(message), file_id=file_id, protect_content=True if pre == 'filep' else False)
            filetype = msg.media
    ...
    await client.send_cached_media(chat_id=get_file_send_chat(message), file_id=file_id, caption=f_caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('❤️‍🔥 ᴊᴏɪɴ ᴛᴏ ᴄʜᴀɴɴᴇʟ ❤️‍🔥', url='https://t.me/+aOWzY1w5cZ44MzJl')]]), protect_content=True if pre == 'filep' else False)
...
async def delete_all_index(bot, message):
    await message.reply_text('This will delete all indexed files.\nDo you want to continue??', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="YES", callback_data="autofilter_delete")],[InlineKeyboardButton(text="CANCEL", callback_data="close_data")]]), quote=True)
...
async def settings(client, message):
    ...
    buttons = [
        ...
        [InlineKeyboardButton('Redirect To', callback_data='noop'), InlineKeyboardButton('Channel (Fixed)', callback_data='noop')],
        ...
        [InlineKeyboardButton('Auto Delete', callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{grp_id}'), InlineKeyboardButton('10 Mins' if settings["auto_delete"] else 'OFF', callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{grp_id}')],
        [InlineKeyboardButton('ShortLink', callback_data=f'setgs#is_shortlink#{settings["is_shortlink"]}#{grp_id}'), InlineKeyboardButton('✅ ON' if settings["is_shortlink"] else '❌ OFF', callback_data=f'setgs#is_shortlink#{settings["is_shortlink"]}#{grp_id}')],
    ]
    await message.reply_text(text=f"<b>Change Your Settings for {title} As Your Wish ⚙</b>", reply_markup=InlineKeyboardMarkup(buttons), disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, reply_to_message_id=message.id)
...
async def deletemultiplefiles(bot, message):
    btn = [[InlineKeyboardButton("Delete PreDVDs", callback_data="predvd"), InlineKeyboardButton("Delete CamRips", callback_data="camrip")]]
    await message.reply_text(text="<b>Select the type of files you want to delete !\n\nThis will delete 100 files from the database for the selected type.</b>", reply_markup=InlineKeyboardMarkup(btn))
...
async def daye(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[InlineKeyboardButton('⏰ ᴛɪᴍᴇ', callback_data='toime'), InlineKeyboardButton('📅 ᴅᴀᴛᴇ', callback_data='datea')]]
        await message.reply(script.DATE_TXT.format(message.from_user.mention if message.from_user else message.chat.title, temp.U_NAME, temp.B_NAME), reply_markup=InlineKeyboardMarkup(buttons))
        await asyncio.sleep(2)
